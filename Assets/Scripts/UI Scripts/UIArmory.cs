...
public class UIArmory : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		string armorDef;
		armorDef = (Inventory.playerArmor.getGem1().getGrade() * 50).ToString () + "% " + Inventory.playerArmor.getGem1 ().getElementS ();
		...
		case 'A':
			...
			GameObject.Find ("DEF").GetComponent<TMPro.TextMeshProUGUI> ().color = Color.blue;
				break;
			default:
			GameObject.Find ("DEF").GetComponent<TMPro.TextMeshProUGUI> ().color = Color.white;
				break;
		}
	}
	void updateDamage()
